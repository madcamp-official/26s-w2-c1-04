...
| 페이지네이션 | 커서 방식. `?before=<id>&limit=30`. 응답에 `next_before` |
...
```
→ `200` + 위와 같은 그룹 객체. 정원 초과면 `409 group_full`, 이미 멤버면 `409 already_member`.
...
> **계산 필드에 주의.** `reply_count`(`parent_id`로 세는 값), `viewed_by_me`(`doodle_receipts` 조인), `thumb_url`(`photo_url`에서 유도)은 **DB 컬럼이 아니다.** 서버가 조립해서 내려준다. `doodles` 테이블에는 없다([ERD.md](ERD.md)).
...
- `doodle_receipts`에 최초 확인 시각을 기록한다. `UNIQUE(doodle_id, user_id)`가 중복을 막으므로 **몇 번 불러도 안전하다**(멱등).
- `mode='ephemeral'`이고 **보낸 사람이 내가 아니면** `expires_at = now + 5s`를 세팅하고 5초 타이머를 건다.
...
| 이벤트 | 페이로드 | 기능 |
|---|---|---|
| `doodle:new` | `{doodle_id, sender_id, mode, content_type, thumb_url, created_at}` | SD-5, SD-6 |
