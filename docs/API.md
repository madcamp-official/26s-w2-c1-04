...
>
> **Flutter 로 바로 붙이려면 → [FRONTEND.md](FRONTEND.md)** (연결값·소켓 코드·온보딩 흐름·체크리스트). 서버는 현재 라이브다.
>
> **현황(2026-07-13).** 서버·GPU 라이브. 프론트는 Claude 디자인 시안을 원본으로 `front-remake` 브랜치에서 0부터 재구축 중이며 현재 Mock 구동(실서버 미연결)이다. 디자인이 요구하지만 아직 서버에 없는 계약은 문서 끝 **"디자인 갭 — 추가 예정 엔드포인트"** 에 모았다. **1~12절의 기존 계약은 그대로 유효하다.**
...
| 413 | `payload_too_large` | 이미지 또는 `stroke_data` 제한 초과 |
...
> 같은 `device_uid`로 다시 부르면 같은 유저에게 **새 토큰을 발급하고 이전 토큰을 무효화한다.** 서버에는 토큰 원문이 아니라 해시만 있어 원래 토큰을 다시 돌려줄 수 없기 때문이다. 재설치로 `device_uid` 자체가 바뀌는 경우는 복구하지 못한다. 소셜 로그인을 나중에 붙이면 같은 `user_id`에 `provider='kakao'` 행을 더한다. [SPEC.md](SPEC.md) 6.6절.
...
이미지는 실제 바이트 형식을 검사한다. `photo`는 JPEG/PNG, `drawing`은 PNG만 허용한다. 파일당 최대 10MB·한 변 최대 8192px, `text_body`는 최대 4000자, `stroke_data`는 UTF-8 기준 최대 512KB다. 손상 이미지와 아래 스키마에 맞지 않는 획 데이터는 `422`, 크기 초과는 `413`이다.
...
> **`thumb_url`은 유형별로 다르게 만든다.** 일반 낙서와 발신자·열람 완료 응답에서는 절대 `null`이 되지 않는다. 단, 수신자가 아직 열지 않은 사라지기 낙서는 내용 보호를 위해 `null`이다.
>
> | `content_type` | 썸네일 원본 |
> |---|---|
...
**사라지기 모드로 삭제된 낙서는 목록에 나오지 않는다.** 아직 확인 전인 `ephemeral` 낙서는 메타데이터만 나오며, 수신자 응답의 `photo_url`·`drawing_url`·`thumb_url`·`text_body`는 모두 `null`이다. `POST /doodles/{id}/view` 성공 뒤 다시 조회하면 내용이 채워진다. 발신자에게는 처음부터 내용이 보인다.

### `GET /doodles/{doodle_id}` 🔒 — RV-4

이미 만료됐으면 `410 doodle_expired`.

수신자가 아직 열지 않은 사라지기 낙서는 목록과 동일하게 내용 필드가 `null`이다.
...
```json
{ "expires_at": "2026-07-10T14:03:05.381642Z" }
```
...
> **`viewed_at`과 `expires_at`은 `DATETIME(6)`이다.** 마이크로초를 보존하지 않으면 5초 TTL이 실제로는 4~5초가 된다. 최초 확인과 재확인은 같은 `expires_at`을 돌려주며, 앱은 이 값으로 카운트다운을 그린다.
...
**성장 규칙.** `exp`는 누적값이다. 일반 낙서 +10, 답장 +5, 찌르기 +2, 쓰다듬기 +1이며 `level = exp // 100 + 1`이다. 레벨이 오를 때 오른 단계당 코인 50개를 주고 룸에 `pet:levelup`을 보낸다. 수치는 환경변수(`PET_*`)로 조정할 수 있다.
...
**일기는 00:05 KST 배치로 생긴다.** 활동도 해당 KST 달력 날짜의 UTC 반개구간으로 조회한다. 가입 첫날 첫 자정 이전에는 일기장이 비어 있고, 그때 `{"items": [], "next_before": null}`이 내려간다. 앱은 이 상태를 정상으로 그려야 한다 — 에러가 아니다.
...
`YYYY-MM` 형식이 아니면 `400 invalid_request`. 응답은 `GET .../reports/{YYYY-MM}` 와 같은 모양이다.

> **구현 노트 두 가지.**
>
> ① **집계 수(`*_count`)는 그 달에 만들어진 모든 낙서를 센다 — 사라지기 모드로 보낸 것도 포함한다.** 보냈다는 사실 자체가 그 달의 활동이기 때문이다. 반면 **`best_doodle` 후보에서는 사라지기 모드 낙서를 통째로 뺀다(SPEC 6.5).** 만료된 것은 파일이 지워져 못 띄우고, 아직 안 열려 안 지워진 것도 '확인 전 노출 금지'(SD-6) 불변식 때문에 뺀다 — 최고의 낙서로 뽑아 뷰어에 띄우면 받는 사람이 열지도 않은 사라지기 낙서 내용이 새 버린다. 그래서 어떤 달은 `drawing_count`가 3인데 최고의 낙서 후보는 1개일 수 있다. 의도된 비대칭이다.
>
> ② **`pet_level_start`는 이력 테이블이 없어 "지난 달 레포트의 `pet_level_end`"로 잡는다.** 첫 레포트(지난 달 레포트가 없음)면 `start = end`(현재 레벨)로 두어 증가폭이 0이 된다. 레포트를 달 순서대로 생성하면 자연스럽게 이어진다. 과거 달을 뒤늦게 생성하면 `pet_level_end`가 "그 달 말"이 아니라 "생성 시점의 현재 레벨"이 된다 — 데모에선 이번 달을 생성하므로 문제되지 않는다.
...
`doodle_received`와 `poke`는 보낸 사람을 제외한 상대 기기에만, `monthly_report`와 만료 후 `widget_refresh`는 그룹 두 사람의 등록 기기에 보낸다. 전송은 Android high priority이며, Firebase 장애는 이미 커밋된 REST 요청을 실패로 바꾸지 않는다. `FCM_CREDENTIALS_PATH`가 없으면 개발용 비활성 구현으로 동작한다.
...
vLLM 0.24.0에서는 OpenAI 호환 `response_format`을 쓴다. 폐기된 `guided_json`은 보내지 않는다.

```json
{
  "response_format": {
    "type": "json_schema",
    "json_schema": { "name": "memory_pager_response", "schema": {} }
  }
}
```
...
| Method | Path | 설명 |
|---|---|---|
| `GET` | `/health` | 준비 완료·로딩·실패 상태. 로딩/실패는 HTTP 503 |
| `POST` | `/generate/diary` | 일기 그림 1장 생성 |
| `POST` | `/train/lora` | P2 계획. 현재 미구현 |
| `GET` | `/train/lora/{job_id}` | P2 계획. 현재 미구현 |
...
`style.kind=learned`는 PT-5/PT-6b(P2)이므로 현재 `501`이다. P1은 `default`만 사용한다. 실제 worker 실패 시 앱 VM은 빈 이미지를 일기로 저장하지 않고 그날 일기 생성을 건너뛴다. `SD_STUB=true`에서는 같은 계약으로 눈에 보이는 512px 테스트 PNG를 만든다.

> **동기 호출로 간다.** 생성이 10~30초 걸리므로 `httpx` 타임아웃을 넉넉히 잡는다. 자정 배치라 지연이 문제되지 않는다. 만약 타임아웃을 넘기면 잡 큐 + 콜백으로 바꾼다.
>
> sd-worker는 SD를 올리기 전에 vLLM에 `POST /sleep`을 보낼 필요가 **없다.** 상주 LLM(~8.4GB) + SD 추론(4~6GB) = 12~14GB로 24GB에 여유롭게 들어간다([STACK.md](STACK.md) 2절). LoRA 학습(`/train/lora`)만 sleep이 필요하다.
...
> **전부 미구현·후보다.** 2026-07-13 기준, `front-remake` 브랜치의 Claude 디자인 시안이 요구하지만 현재 서버에 없는 계약이다. 확정 전까지 앱은 Mock으로 대체한다. **위 1~12절의 기존 계약은 이 섹션과 무관하게 그대로 유효하며**, 아래 항목이 확정되면 해당 절을 고쳐 반영한다.

### (a) 오늘의 질문 — 미구현·후보

디자인이 새로 도입한 기능. 매일 커플에게 **같은 질문 하나**가 내려가고 각자 답한다. 현재 백엔드에 없다.

`GET /groups/{group_id}/question/today` 🔒 — 오늘 질문과 상대 답변 여부
...
아직 안 썼으면 `my_answer: null`. 상대 답변 원문을 언제 공개할지(내가 먼저 답해야 보이는 동시 공개 방식 등)는 미확정이라, 우선 `partner_answered`(불리언)만 노출해 "상대는 이미 답했어요" 힌트를 준다.

`POST /groups/{group_id}/question/today` 🔒 — 내 답변 저장

```json
{ "answer": "네가 챙겨준 아침" }
```
→ 위 GET과 같은 모양. 재호출 시 수정 허용 여부는 미확정.

### (b) 그룹 응답에 `created_at` 노출 — 미구현·후보

디자인 홈이 사귄 날부터의 **D-day**를 그린다. 현재 그룹 객체(3절)에는 `created_at`이 없다. 후보: 그룹 응답에 `"created_at": "..."` 필드를 **추가**한다. 기존 필드는 유지.

### (c) `groups/join` 응답을 `{group, pet}`로 래핑 — 미구현·후보

현재 `POST /groups/join`(3절)은 그룹 객체를 **bare**로 돌려준다. 디자인은 가입 직후 펫을 함께 보여주므로, `POST /groups`와 동일하게 `{ "group": {...}, "pet": {...} }` 래핑이 필요하다. **확정 시 3절 계약을 이 모양으로 교체한다** — 그 전까지 앱은 bare group 응답을 파싱한다.

### (d) 앨범 AI 큐레이션 — 미구현·후보 (P2)

"모리가 모아줬어요" — 펫이 사진첩에서 낙서 몇 개를 골라 묶어 준다. 후보: `GET /groups/{group_id}/albums/curated` 🔒 → 큐레이션 묶음 목록(제목 + 낙서 id 배열). 선정 규칙은 미정.

### (e) 낙서 텍스트 검색 — 미구현·후보 (P2)

낙서 속 단어로 사진첩을 검색한다. 후보: 기존 `GET /groups/{group_id}/doodles`(4절)에 `?q=<검색어>` 쿼리를 더해 `text_body`(및 향후 OCR 결과)를 매칭한다. 현재 필터(`content_type`, `date`)와 병용하는 **가산 변경**이라 기존 계약을 깨지 않는다.
